...
export default async function handler(ctx: InvokeContext): Promise<InvokeResult> {
  ...
  try {
    switch (verb) {
      case "verify":
        if (!positional[0]) return done(false, "usage: maw crew verify <prefix>");
      ...
      case "status":
        ...
        return done(true);
      case "up": {
        if (!positional[0]) return done(false, "usage: maw crew up <name> --pools 1,5");
      ...
      case "down":
        if (!positional[0]) return done(false, "usage: maw crew down <prefix>");
    ...
    const msg = e instanceof CrewError ? `crew: ${e.message}` : String((e as Error)?.stack ?? e);
    return done(false, msg);
  }
}

// maw EXECUTES this file rather than importing it, so the default export alone
// would never run. This block is what actually serves `maw crew ...`.
if (import.meta.main) {
